...
namespace ALCHIMED
{
    ...
    class Joueur : Personnage
    {
        ...
        //propriétés
        public int NbSubstances {
            get { return _inventaireSubstances.Count; }
        }
         ...
         internal void Defier(Personnage adversaire, int coup)
         {
              ...
              if (coup == 2) // si le joueur a choisi de répliquer par un coup de poing
             {
                 ...
                 else
                 {
                     ...
                     Console.Write("\nVous attaquez " + adversaire._nom + " qui perd " + this._ptForce / 6 + " points de vie.");
                     if (adversaire._ptVie <= 0)
                     { Console.WriteLine("Il ne lui reste aucun point de vie.\n"); }
                     else
                     { Console.WriteLine("Il lui reste " + adversaire._ptVie + " points de vie.\n"); }
        ...
        // Procédure permettant de déplacer le joueur directement dans le lit de la salle de repos
         // En entrée : l'ObjetSpecial ayant l'effet regenerateur (ici lit)
    ...
    public bool Deplacer(Piece lieu)
        {
            ...
            if (etatAcces != Acces.Autorise)
            {
                ...
                if (etatAcces == Acces.RefuseEntrainement)
                    Console.WriteLine("\nAccès refusé : vous n'êtes pas pas encore assez fort.\n");
                else if (etatAcces == Acces.RefuseEtude)
                    Console.WriteLine("\nAccès refusé : vous n'avez pas suffisamment étudié.\n");
            }
        ...
        public void DeplacerVersObjet(Piece lieu, int choix)
        {
        ...
        /*Méthode permettant d'afficher les substances que possède le joueur dans son inventaire
        */
        public void AfficheInventaire()
        {
            string contenu = "";
            if (_inventaireSubstances.Count > 0)
            {
                contenu = "\n Substances :";
                foreach (Substances extrait in _inventaireSubstances)
                    contenu += "\n " + extrait;
            }
            else {
                contenu += " l'inventaire est vide";
            }
            Console.WriteLine("\nInventaire :"+contenu);
        }
