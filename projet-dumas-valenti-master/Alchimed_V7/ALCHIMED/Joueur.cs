...
namespace ALCHIMED
{
    //Enumération des Savoirs pouvant être possédés par un joueur
    // et/ou enseignés par un objet
    public enum Savoirs
    {
        Culture,
        Chimie,
    ...
    class Joueur : Personnage
    {
        ...
        */
        ...
        // Se déplacer vers une autre pièce si permission d'y entrer
        public void Deplacer(Piece lieu)
        {
            //vérification des conditions d'entrée et résultat dans une variable de type énuméré
            Acces etatAcces = lieu.Acceder(_savoirs, _ptForce);
            if (etatAcces != Acces.Autorise)
            {
                //Affichage des raison du refus
            ...
            {
                //Accès autorisé, description de la pièce
        ...
        {
            //Présenter les objets de la pièce et proposer d'interagir avec l'un d'eux
            lieu.Lister();
            int choix = int.Parse(Console.ReadLine());

            foreach (Objet objet in lieu._objets)
                {
                    //Recherche de l'objet choisi
                    if (objet.Num == choix)
                    {
                        //Si trouvé, affichage de sa description
                        objet.AfficheDescription();
                    
                        // recherche d'un Enseignement possible
                        if (objet.Verif() == true)
                        {
                            //Si trouvé, vérification que le joueur ne le possède pas encore
